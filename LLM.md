...
| `ringtailfx` | Ringtail threshold (PQ) | Verify PQ threshold sigs from T-Chain MPC | TODO |
| `ringfx` | LSAG ring signatures | Sender anonymity (Monero-style) | TODO |
| `kemfx` | ML-KEM-768 | Stealth addresses, encrypted memos | TODO |

**Threshold signing architecture:** X-Chain Fx plugins VERIFY threshold signatures. The actual threshold signing ceremonies happen on T-Chain (MPC runtime). So `cggmp21fx`/`frostfx`/`ringtailfx` are verifier plugins that accept the final aggregated sig — the keygen + signing rounds live on T-Chain.
